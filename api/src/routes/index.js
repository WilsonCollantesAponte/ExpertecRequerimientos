...
router.put("/requirementAdmin", async (req, res) => {
  ...
  // const Admin = await Requirement.findOne({ where: { id } });

  // if (Admin.estadoFinalAdmin) {
  //   Admin.estadoFinalAdmin = false;
  //   Admin.save();
  // } else {
  //   Admin.estadoFinalAdmin = true;
  //   Admin.save();
...
router.get("/allDev", async (req, res) => {
  try {
    ...
    if (!name) {
      ...
      //     },
      //   },
      // });  Forma de la documentación --Referencia 1
...
router.put("/reqUserUpdate", async (req, res) => {
  try {
    const { id, requerimientos } = req.body;

    await User.update({ requerimientos }, { where: { id } });
